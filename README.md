...
- iOS builds on Xcode 27 need an iOS 15 deployment target. Set the Runner
  ...
  package, so `hand_detection` cannot set it for you.
...
void _onFrame(CameraImage image) {
  _throttle.run(() async {
    ...
    if (mounted) setState(() { this.hands = hands; imageSize = size; });
  });
}
```
...
The [example app](https://pub.dev/packages/hand_detection/example) ships a complete, working live-camera screen
